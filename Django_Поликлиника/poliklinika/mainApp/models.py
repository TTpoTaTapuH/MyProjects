...
class MedSister(models.Model): #Мед сестра
    ...
    telephone = models.CharField(max_length=12, unique=True, verbose_name='Телефон')
    work_plan = models.OneToOneField(WorkGrafic, on_delete=models.PROTECT, null=True, verbose_name='График работы', blank=True)
    doctor = models.ForeignKey(Doctor, on_delete=models.PROTECT, verbose_name='Медсестра', null=True, blank=True)
    def __str__(self):
        namea = self.surname+' '+self.name+'.'+self.second_name
        return namea
    class Meta:
...
class Draft(models.Model):
    ...
    class Meta:
        ...
        ordering = ['date']
class TimeReception(models.Model):
    list_status=(
        ("Free","Свободен"),
        ("Busy","Занят"),
    )
    ...
    week_day = models.CharField(max_length=15, default='Понедельник', choices=list_day, verbose_name='День недели')
    doctors = models.ManyToManyField(Doctor, verbose_name='Врач', related_name='time_reception')
    def __str__(self):
        a = self.week_day+' '+self.datetime.strftime("(%H:%M)")
